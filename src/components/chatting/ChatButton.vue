<template>
  ...
  <div class="fixed bottom-[15px] right-[15px]">
    <div
      ...
      <span v-if="rooms.some((room) => room.hasNewMessage)"
  ...
  <div v-if="chatStore.isChatBoxToggled"
    ...
    <div class="w-1/4 h-full bg-white border-r panel-divider">
      ...
      <div v-for="(room, idx) in rooms" :key="room.chatRoomId" @click="selectRoom(room.chatRoomId, idx)"
        ...
        <div class="flex items-center gap-md">
          <img class="w-[40px] h-[40px] rounded-full bg-gray-200" :src="room.userImg || '/default-profile.png'"
            alt="프로필 이미지" />
          <div class="truncate">
    ...
    <!-- 채팅 패널 구역 -->
    <ChatPannel class="w-3/4 h-full bg-white" :roomId="roomId" :chatLogs="chattings" :receiver="receiver"
      :sender="sender" />
  </div>
</template>
<script setup>
